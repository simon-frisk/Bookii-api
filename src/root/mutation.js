...
const validateUser = async user => {
  ...
  if (!passwordRegex.test(user.password)) throw new UserInputError('Password too short')
  if (await User.findOne({ email: user.email })) throw new UserInputError('Email already in use')
}
